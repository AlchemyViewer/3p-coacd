...
apply_patch "$top/patches/coacd-prebuilt-and-build.patch" "$COACD_SOURCE_DIR"

pushd "$COACD_SOURCE_DIR"
    case "$AUTOBUILD_PLATFORM" in
        ...
        windows*)
            ...
            mkdir -p "build_debug"
            pushd "build_debug"
                opts="$(replace_switch /Zi /Z7 $LL_BUILD_DEBUG) -DOPENVDB_STATICLIB=1"
                plainopts="$(remove_switch /GR $(remove_cxxstd $opts))"

                cmake -G "$AUTOBUILD_WIN_CMAKE_GEN" -A "$AUTOBUILD_WIN_VSPLATFORM" .. -DBUILD_SHARED_LIBS=OFF \
                        -DPREBUILT_LIB_DIR="$(cygpath -m "$stage/packages/lib/debug")" \
                        -DPREBUILT_INCLUDE_DIR="$(cygpath -m "$stage/packages/include")" \
            ...
            pushd "build_release"
                opts="$(replace_switch /Zi /Z7 $LL_BUILD_RELEASE) -DOPENVDB_STATICLIB=1"
                plainopts="$(remove_switch /GR $(remove_cxxstd $opts))"

                cmake -G "$AUTOBUILD_WIN_CMAKE_GEN" -A "$AUTOBUILD_WIN_VSPLATFORM" .. -DBUILD_SHARED_LIBS=OFF \
                        -DPREBUILT_LIB_DIR="$(cygpath -m "$stage/packages/lib/release")" \
                        -DPREBUILT_INCLUDE_DIR="$(cygpath -m "$stage/packages/include")" \
        ...
        darwin*)
            ...
            lipo -create -output "$stage/lib/release/libcoacd.a" "$stage/lib/release/x86_64/libcoacd.a" "$stage/lib/release/arm64/libcoacd.a"
        ;;
    ...
    mkdir -p "$stage/include/coacd/"
    cp public/*.h "$stage/include/coacd/"

    mkdir -p "$stage/LICENSES"
    cp LICENSE "$stage/LICENSES/coacd.txt"
popd
